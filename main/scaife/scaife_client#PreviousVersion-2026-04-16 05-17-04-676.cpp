...
typedef struct {
    ...
    int   written;
    bool  overflow;
    bool  capture_link;   // true when fetching JSON (need Link header)
} fetch_ctx_t;
...
static esp_err_t http_event_handler(esp_http_client_event_t *evt)
{
    ...
    case HTTP_EVENT_ON_HEADER:
        // Capture the Link header when requested
        if (ctx->capture_link &&
...
static int scaife_fetch(const char *url, char *buf, int buf_size,
                        bool capture_link)
{
    fetch_ctx_t ctx = { buf, buf_size, 0, false, capture_link };
    buf[0] = '\0';
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        ESP_LOGI(TAG, "attempt %d: err=%d status=%d bytes=%d",
                 attempt, err, status, ctx.written);

        if (err == ESP_OK && status == 200 && ctx.written > 0) {
            if (ctx.overflow)
...
int scaife_get_passage_json(const char *full_urn, char *buf, int buf_size, bool root)
{
    char url[256];
    if (root){snprintf(url, sizeof(url), "https://scaife.perseus.org%s", full_urn);} 
    else {    snprintf(url, sizeof(url), "https://scaife.perseus.org/library/passage/%s/json/", full_urn);}
    ESP_LOGI(TAG, "GET JSON: %s", url);
    return scaife_fetch(url, buf, buf_size, true);
}
